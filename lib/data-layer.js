...
export function buildMasterStockObject(core, extra={}){
  ...
  const integrity=core?.integrity||{};
  const indScores=indicators?.scores||{};
  const indValues=indicators?.values||{};
  const symbol=cleanSymbol(core?.symbol || extra.symbol);
  const day=clamp(d.score ?? d.scores?.day ?? avg([indScores.volume,indScores.momentum,indScores.institutional],0));
  const swing=clamp(avg([a.trend,a.momentum,a.money,inst.components?.trend,inst.components?.momentum,indScores.trend,indScores.momentum], a.finalScore||0));
  const position=clamp(avg([q.components?.trend,q.components?.backtest,q.components?.risk,q.components?.liquidity,extra.financialScore,indScores.quality], q.score||a.confidence||0));
  const institutional=clamp(inst.score ?? avg([inst.components?.moneyFlow,inst.components?.rvol,inst.components?.vwap,inst.components?.relativeStrength,indScores.institutional,indScores.smartMoney], a.money||0));
  const iqs=clamp(q.score ?? indScores.quality ?? avg([a.trend,a.money,a.momentum,100-(a.risk||50)], a.finalScore||0));
  ...
  return {
    ...
    layers:{analysis:a, indicators, integrity, dayTrading:d, institutional:inst, quality:q, v19},
    metrics:{rvol20:d.rvol20 ?? indValues.rvol20, rvol5:d.rvol5 ?? indValues.rvol5, vwap20:d.vwap20 ?? indValues.vwap20, cmf20:d.cmf20 ?? indValues.cmf20, mfi14:d.mfi14 ?? indValues.mfi14, rsi14:d.rsi14 ?? indValues.rsi14, atrPct:d.atrPct ?? indValues.atrPct, adx14:indValues.adx14, macdHist:indValues.macdHist, obvSlopeScore:indValues.obvSlopeScore, indicatorCount:indicators.count||0, dataHealth:integrity.health, volumeDays:integrity.values?.positiveVolumeDays, dataIssues:integrity.issues||[], dipDistance},
    integrity,
    meta:{source:'R11 Data Integrity + R1 Data Layer', random:false, computedAt:new Date().toISOString(), core:core.meta||{}}
  };
}
