...
export async function getCoreAnalysis(symbol, options={}){
  ...
  const payload={symbol:sym, rows, analysis:base, dayTrading, institutional, v19, quality, meta:{source:'V26 Core Engine', random:false, computedAt:new Date().toISOString(), range, benchmark}};
...
export function compactCoreResult(core){
  ...
  return {symbol:core.symbol, close:round(a.close), change:round(a.change), decision:a.decision, finalScore:a.finalScore, confidence:a.confidence, risk:a.risk, trend:a.trend, money:a.money, momentum:a.momentum, formation:a.formation?.name||'Belirgin formasyon yok', dayTrading:d, institutional:i, quality:q, meta:core.meta};
}
