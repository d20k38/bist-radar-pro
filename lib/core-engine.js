...
import { assessDataIntegrity } from './data-integrity-engine.js';
...
export async function getCoreAnalysis(symbol, options={}){
  ...
  const benchmark=(options.benchmark===false || options.benchmark===null) ? null : (options.benchmark || 'XU100');
  const key=[sym,range,benchmark||'none', options.includeV19?'v19':'std'].join(KEY_SEP);
  const s=getStore();
  ...
  let benchmarkRows=[];
  if(benchmark){ try{ benchmarkRows=await getCoreOhlcv(benchmark,range,'1d'); }catch(e){ benchmarkRows=[]; } }
  const indicators=buildIndicatorBundle(rows, benchmarkRows);
  const dayTrading=analyzeDayTrading(rows, benchmarkRows);
  const integrity=assessDataIntegrity(rows, indicators, dayTrading);
  let v19=null;
  ...
  const payload={symbol:sym, rows, analysis:base, indicators, integrity, dayTrading, institutional, v19, quality, meta:{source:'R11 Core + Data Integrity + Indicator Engine', random:false, computedAt:new Date().toISOString(), range, benchmark}};
