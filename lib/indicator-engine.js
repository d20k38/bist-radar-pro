...
function relativeVolume(v,n=20){
  // R10.2: 0/null hacimler RVOL'u 0x'e düşürmesin.
  // Her nokta için önceki n pozitif hacmin ortalaması kullanılır; yetersizse null.
  const out=Array(v.length).fill(null);
  for(let i=0;i<v.length;i++){
    const cur=Number(v[i]);
    if(!Number.isFinite(cur)||cur<=0) continue;
    const prev=[];
    for(let j=i-1;j>=0 && prev.length<n;j--){const x=Number(v[j]); if(Number.isFinite(x)&&x>0) prev.push(x);}
    if(prev.length>=Math.min(5,n)){const av=prev.reduce((a,b)=>a+b,0)/prev.length; out[i]=av?cur/av:null;}
  }
  return out;
}
...
export function buildIndicatorBundle(rows, benchmarkRows=[]){
  ...
  const o=series(rows,'open'),h=series(rows,'high'),l=series(rows,'low'),c=series(rows,'close'),v=series(rows,'volume').map(x=>Number.isFinite(Number(x))&&Number(x)>0?Number(x):null);
