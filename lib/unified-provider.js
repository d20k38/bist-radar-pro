...
const R19Decision = require('./decision-engine');
...
function scoreFromIndicators(ind){
  return R19Decision.scoreFromIndicators(ind || {});
}

function providerFreshness(hist, quote, rows){
  const last = (rows||[]).length ? rows[rows.length-1] : {};
  const lastBarDate = last.date || null;
  let ageDays = null;
  if(lastBarDate){
    const t = new Date(lastBarDate + 'T23:59:59Z').getTime();
    if(Number.isFinite(t)) ageDays = Math.max(0, Math.floor((Date.now()-t)/86400000));
  }
  const provider = hist?.provider || 'unknown';
  const quoteProvider = quote?.provider || 'quote-unavailable';
  const realTime = false;
  const status = ageDays==null ? 'Tarih bilinmiyor' : ageDays<=1 ? 'Güncel / son seans' : ageDays<=4 ? 'Gecikmeli / son kapanış' : 'Eski veri uyarısı';
  return {
    provider,
    ...
    realTime,
    status,
    mode:'Ücretsiz veri sağlayıcı; BIST gerçek zamanlı lisanslı veri değildir, gecikmeli veya son kapanış olabilir.',
    rows:(rows||[]).length,
    positiveVolumes:(rows||[]).filter(r=>Number.isFinite(Number(r.volume)) && Number(r.volume)>0).length,
    providerErrors: hist?.fallbackFrom || quote?.fallbackFrom || [],
...
async function master(symbol, opts={}){
  ...
  const dataProvider = providerFreshness(hist.value, q, rows);
  return { success:true, symbol:String(symbol).toUpperCase(), name:q.name||hist.value.meta?.longName||symbol, price: q.price || ind.lastPrice, change:q.change, changePercent:q.changePercent, ohlcv:rows, indicators:ind, scores, decision:scores.decision, score:scores.finalScore, aiScore:scores.finalScore, finalScore:scores.finalScore, confidence:scores.confidence, confidencePct:scores.confidencePct, risk:scores.risk, iqs:scores.iqs, dayTrading:scores.dayTrading, swing:scores.swing, position:scores.position, institutional:scores.institutional, explain:scores.explain, breakdown:scores.breakdown, dataHealth:ind.health, dataProvider, provider:hist.value.provider || 'R27 provider', providerErrors:hist.value.fallbackFrom || [], quoteProvider:q.provider, timestamp:new Date().toISOString() };
}
...
module.exports = { SYMBOLS, fetchOHLCV, fetchQuote, indicators, scoreFromIndicators, master, clamp, diagnose, diagnoseMany, providerInfo: { primary:yahooFinance2?'yahoo-finance2':'none', fallback:'yahoo-http-chart', decision:'R19 AI Decision Engine', version:'R27 Multi Provider' } };
