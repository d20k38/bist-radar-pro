...
async function dynamicKapSymbols(){
  ...
  try{
    ...
    return (found.length >= 500 && found.length <= 850) ? found : []; // R1: aşırı geniş KAP regex sonuçları sembol evreni sayılmaz
  }catch(e){ return []; }
...
export async function getSymbols(){
  ...
  const best = (dynamic.length >= 500 && dynamic.length <= 850 && dynamic.length > local.length) ? dynamic : local; // R1: canonical universe; 1500+ gürültülü liste kullanılmaz
