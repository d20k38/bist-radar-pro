...
export default async function handler(req,res){
  ...
  for(const symbol of symbols){
    try{ const rows=await getOhlcv(symbol,'1y','1d'); const a=analyze(rows); data.push({symbol,close:a.close,change:a.change,decision:a.decision,finalScore:a.finalScore,confidence:a.confidence,risk:a.risk,trend:a.trend,money:a.money,momentum:a.momentum,potential:a.potential}); }
