...
export default async function handler(req,res){
  ...
  for(const symbol of symbols){
    try{ const rows=await getOhlcv(symbol,'1y','1d'); const a=analyze(rows); data.push({symbol,close:Number(a?.close)||0,change:Number(a?.change)||0,decision:a?.decision||'VERİ YOK',finalScore:Number(a?.finalScore)||0,confidence:Number(a?.confidence)||0,risk:Number(a?.risk)||100,trend:Number(a?.trend)||0,money:Number(a?.money)||0,momentum:Number(a?.momentum)||0,potential:Number(a?.potential)||0,pattern:Number(a?.pattern)||0,safeScore:Number(a?.safeScore)||0,opportunityScore:Number(a?.opportunityScore)||0,riskReward:Number(a?.riskReward)||0,formation:a?.formation?.name||''}); }
